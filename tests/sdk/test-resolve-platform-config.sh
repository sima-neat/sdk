...
Version: 3.0.0~git202609090314.abcdef0-4040
Architecture: arm64
...
Package: simaai-palette-modalix
Version: 3.0.0~git202609090314.abcdef0-4460
Architecture: arm64

Package: simaai-palette-modalix
Version: 3.0.1~git202609090314.abcdef0-9999
Architecture: arm64
EOF
...
Package: simaai-palette-modalix
Version: 3.0.0~git202609090314.abcdef0-4593
Architecture: arm64
EOF
...
by_hash_floating="$(
  PRE_RELEASE_BASE=3.0.0 \
    ...
    PRE_RELEASE_RELEASE_URL=https://mirror.test/daily/dists/agate/Release \
    CURL_COMMAND="${tmpdir}/curl" \
    GITHUB_REF_TYPE=branch \
    GITHUB_REF_NAME=develop \
    run_resolver
)"
grep -Fxq 'base_sdk_version=3.0.0~git202609090314.abcdef0-4593' <<< "${by_hash_floating}" || \
  fail "floating selector did not use the current by-hash package index"

by_hash_pinned="$(
  PRE_RELEASE_BASE=3.0.0~git202609090314.abcdef0-4460 \
    PRE_RELEASE_PACKAGES_FILE= \
    PRE_RELEASE_RELEASE_URL=https://mirror.test/daily/dists/agate/Release \
    CURL_COMMAND="${tmpdir}/curl" \
    GITHUB_REF_TYPE=branch \
...
grep -Fxq 'base_sdk_version=3.0.0~git202609090314.abcdef0-4460' <<< "${by_hash_pinned}" || \
  fail "exact pre-release selector did not remain pinned"

pinned="$(PRE_RELEASE_BASE=3.0.0~git202609090314.abcdef0-4040 GITHUB_REF_TYPE=branch GITHUB_REF_NAME=main run_resolver)"
grep -Fxq 'base_sdk_version=3.0.0~git202609090314.abcdef0-4040' <<< "${pinned}" || fail "pinned selector changed versions"

for protected_ref in main release-2.1; do
  if PRE_RELEASE_BASE=3.0.0 GITHUB_REF_TYPE=branch GITHUB_REF_NAME="${protected_ref}" run_resolver >"${tmpdir}/out" 2>"${tmpdir}/err"; then
    fail "floating selector was accepted for protected branch ${protected_ref}"
  fi
  grep -Fq 'floating PRE_RELEASE_BASE=3.0.0 is not allowed' "${tmpdir}/err" || fail "protected branch error was unclear"
done

if PRE_RELEASE_BASE=3.0.0 GITHUB_REF_TYPE=tag GITHUB_REF_NAME=v3.0.0 run_resolver >"${tmpdir}/out" 2>"${tmpdir}/err"; then
  fail "floating selector was accepted for a tag"
fi

if PRE_RELEASE_BASE=3.0.0~git202609090314.abcdef0-9999 GITHUB_REF_TYPE=branch GITHUB_REF_NAME=develop run_resolver >"${tmpdir}/out" 2>"${tmpdir}/err"; then
