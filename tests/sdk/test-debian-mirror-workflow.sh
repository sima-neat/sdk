...
apt_mirror2_installer="${repo_root}/scripts/install-apt-mirror2.sh"
...
grep -Fq 'scripts/install-apt-mirror2.sh' "${workflow}"
grep -Fq 'apt-cache show apt-mirror2' "${apt_mirror2_installer}"
grep -Fq '22.04|24.04|26.04' "${apt_mirror2_installer}"
grep -Fq 'signed-by=${PACKAGECLOUD_KEYRING}' "${apt_mirror2_installer}"
grep -Fq 'DDE9C7D7C3EEBA0B18758002D8DAB1FC88E4447F' "${apt_mirror2_installer}"
grep -Fq 'ln -s "$(command -v apt-mirror)" /usr/local/bin/apt-mirror2' "${apt_mirror2_installer}"
